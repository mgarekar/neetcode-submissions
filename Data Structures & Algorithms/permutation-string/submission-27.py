...
class Solution:
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        for r in range(len(s1), len(s2)):
            ...
            # Character coming into the window (right)
            rchar = s2[r]
            hm_s2[rchar] += 1
            if hm_s1[rchar] == hm_s2[rchar]:
                matches += 1
            elif hm_s1[rchar] + 1 == hm_s2[rchar]:
                matches -= 1

            # Character going out of the window (left)
            lchar = s2[l]
            hm_s2[lchar] -= 1
            if hm_s1[lchar] == hm_s2[lchar]:
                matches += 1
            elif hm_s1[lchar] - 1 == hm_s2[lchar]:
                matches -= 1
